...
| ~~复现 2~~ | — | 已取消（排队被同账号 veomni job 占满，且复现 1 已足以定论；用户拍板不再补跑） | | | | `evl_b13ed4dfe3e0444caab9eec8dc5c4ac2` |
| (006) 3 次运行 | SGLang 0.5.9 | 0.131 / 0.187 / 0.141 | 各 198/198 | 0 | — | 见 006 |

0.5152 与昨天的 0.5808 相差 6.6 分——单次 GPQA 方差本就有 ±3 分以上（006 里 NVFP4 三次跑出 0.449–0.530，
跨 8 分），两者均为单次运行，属正常波动范围；关键对照是它们都**远高于** SGLang 侧的 0.13–0.19。

**补充证据（vLLM 版 K3 重测，详见 [007](007-k3-kl-divergence-bf16-vs-nvfp4-vs-qad.md) 的"vLLM 重测"一节）：**
纯 vLLM 栈下 QAD 的真实 K3 = 0.031/0.033（对称，甚至低于 NVFP4 的 0.045/0.049），而 sglang 栈下测得
0.19/0.25（虚高 6–7 倍、伪不对称）。NVFP4 各行跨引擎吻合（0.045–0.049 vs 0.047），进一步把缺陷收窄到
"SGLang × 这份 QAD 导出"的组合上。

...
| D（w4a4 + FP4-KV） | 0.5354 / 0.5303 / 0.5303 | **0.5320** |
| E（w4a4 + FP8-KV，生产现状） | 0.5707 / 0.6111 / 0.5758 | **0.5859** |

读数：**D−E = −5.4pp**；E−A = −2.9pp（与 005 的 −3.7pp 交叉吻合）；D−A = −8.3pp。全部 0 错误、提交前过 gate-200 + hook first-call 双自检。注意 E 臂 3-run 方差偏大（0.571–0.611，极差 4pp）而 D 臂极稳（0.530–0.535），D−E 读数含一定 run 间噪声。

### 4.2 RULER（2026-07-05 补齐，与 §2/§3 严格同 harness）
...
- **KV-FP4 边际代价随上下文急剧放大**（−1.0pp @8K → −6.6pp @32K），与 8B/4B 同款模式但幅度更大。
- 重灾任务 @32K（D−E）：`niah_multikey_3` **−20pp**、`ruler_qa_squad` −17.8pp、`ruler_cwe` −9.0pp；`niah_single` 系列仍近满分。
...
## 4B. 长上下文扩展：64K/128K RULER（YaRN，004 §8.2）

- **协议**：三臂统一 YaRN rope-scaling（factor=4，original_max_position=32768，`--context-length 133120` + `SGLANG_ALLOW_OVERWRITE_LONGER_CONTEXT_LEN=1`），nonthink，13 任务 × 50 例，64K 并发 8 / 128K 并发 4，turboeval（同 §2/§3/§4.2 harness）。**YaRN 是独立 protocol 块，数字不与 8K/32K（原生 rope）表混排**；D−E/E−A 臂间差值在块内严格可比。均值为 12 任务（去 vt）。
- 功能自检：A 臂 @64K niah_single 0.96–1.0 → YaRN rope 正常工作。

### 4B.1 Qwen3-30B-A3B

| | A | D | E | **D−E** | E−A |
| --- | --- | --- | --- | --- | --- |
| @64K | 78.91 | 54.07 | 72.24 | **−18.2pp** | −6.7pp |
| @128K | 76.75 | 47.71 | 63.62 | **−15.9pp** | **−13.1pp** |

- **D−E 全长度曲线（30B，turboeval 同 harness）：8K −1.0 → 32K −6.6 → 64K −18.2 → 128K −15.9**。KV-FP4 代价在 64K 进入灾难区；128K 上 D−E 略收窄是因为 **E（FP8-KV）自己也塌了**（E−A −13.1pp），全链路 D−A @128K = **−29.0pp**。
- 重灾任务：@64K `niah_multikey_3` −48pp、`ruler_fwe` −36.7pp、multikey_1/2 各 −26pp；@128K multikey_2 −30、multikey_1 −28、multivalue −24、multiquery −23。多 key/多值检索全面崩溃，niah_single 仍近满分。
- 78/78 job 全 completed、0 trial 错误。

### 4B.2 Qwen3-8B

| | A | D | E | **D−E** | E−A |
| --- | --- | --- | --- | --- | --- |
| @64K | 74.01 | 67.23 | 69.36 | **−2.1pp** | −4.7pp |
| @128K | 71.51 | 62.25 | 64.91 | **−2.7pp** | −6.6pp |

- **D−E 全长度曲线（8B）：8K −0.7 → 32K −2.0 → 64K −2.1 → 128K −2.7**——全程平缓，与 30B 的崩溃式曲线形成鲜明对比（跨模型敏感度差异 >6×）。
- E−A（FP8-KV 代价）随长度稳定增长（−4.7 → −6.6pp）；D−E 重灾 @128K 集中在 `niah_multikey_2`（−26pp），其余任务 ≤4pp。
- 8B 原生长上下文能力本就偏弱（A 臂 @64K multikey_3 仅 0.30），部分子任务存在地板效应，解读以均值与 multikey 系为主。78/78 job 0 错误。

### 4B.3 Qwen3-4B（注意口径：E 臂 = BF16-KV，故 D−E = FP4-vs-BF16 的全额 KV 代价）

| | A | D | E | **D−E** | E−A |
| --- | --- | --- | --- | --- | --- |
| @64K | 71.07 | 56.18 | 64.34 | **−8.2pp** | −6.7pp |
| @128K | 64.66 | 50.09 | 58.69 | **−8.6pp** | −6.0pp |

- **D−E 全长度曲线（4B，FP4-vs-BF16 口径）：8K −0.6 → 32K −2.5 → 64K −8.2 → 128K −8.6**。
- **口径对齐后 dense 双雄互相印证**：8B 的 FP4-vs-BF16 代价（D−A）@64K ≈ −6.8pp / @128K ≈ −9.3pp，与 4B 的 −8.2/−8.6pp 高度一致——**dense 8-KV-head 模型的 KV-FP4 全额代价在 64K+ 稳定在 −7～−9pp**；而 30B 的 D−A @64K = −24.8pp / @128K = −29.0pp，敏感度约 3 倍。
- 4B 的 E−A（纯 w4a4 权重/激活代价，两臂同 BF16-KV）在长上下文放大到 −6~−7pp（短上下文仅 −0.5~+1.3）——llmat w4a4 的损失也随上下文增长，与 KV 无关。
- 重灾任务与其他模型一致（multikey 系 −14~−20pp）。78/78 job 0 错误。
...
| Qwen3-30B-A3B | **−5.4pp** | −1.0pp | **−6.6pp** | turboeval（严格同 harness，07-05 补齐） |

**对照 004 §6 决策门：**

1. **D−E @32K RULER ≥3pp 的门被 30B 决定性触发**（−6.6pp），8B/4B 在 2–2.5pp 的门槛边缘 → **Phase 1 KV-QAT 必要且收益明确，且优先级最高的目标模型是 30B-A3B（MoE/少 KV head）**。
2. **"短上下文掉点小、RULER 32K 掉点大"的模式在三个模型上全部成立**（30B：8K −1.0pp vs 32K −6.6pp）→ 主战场在长上下文，**Phase 1 的训练与评测重心应放 RULER 32K–128K**,短上下文能力基本免费。
...
**长上下文扩展后的完整 D−E 曲线（RULER 12 任务均值,YaRN 区间与原生区间用 ‖ 分隔）：**

| 模型 | 8K | 32K | ‖ | 64K | 128K |
| --- | --- | --- | --- | --- | --- |
| Qwen3-8B（FP4 vs FP8） | −0.7 | −2.0 | ‖ | −2.1 | −2.7 |
| Qwen3-4B（FP4 vs BF16） | −0.6 | −2.5 | ‖ | −8.2 | −8.6 |
| Qwen3-30B-A3B（FP4 vs FP8） | −1.0 | −6.6 | ‖ | **−18.2** | **−15.9**（E 也在塌,D−A=−29.0） |

**长上下文数据对决策门的增补判定：**

5. **64K 是 30B 的断崖点**：D−E 从 32K 的 −6.6pp 跳到 64K 的 −18.2pp;dense 模型（口径对齐为 FP4-vs-BF16）稳定在 −7~−9pp。**KV-QAT 若只救一个模型,救 30B-A3B;若只训一个长度区间,训 32K–128K。**
6. **FP8-KV 也不是长上下文的安全区**：30B 的 E−A 从 32K −2.0pp 恶化到 128K **−13.1pp**,8B 到 −6.6pp——生产现状（FP8-KV）本身在 128K 已有两位数损失,KV-QAT 的收益基线应对照 BF16-KV 而非 FP8-KV。
7. dense 模型（8B/4B）的 FP4 边际代价在 64K+ 趋于饱和（曲线走平）,30B 未饱和——支持"KV head 数少/MoE 结构放大 KV 量化误差"的机理假设（30B 仅 4 个 KV head,8B/4B 为 8 个）,可作为 KV-QAT 架构侧的观察点。

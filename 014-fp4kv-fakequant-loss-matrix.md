...
- **协议**：三臂统一 YaRN rope-scaling（factor=4，original_max_position=32768，`--context-length 133120` + `SGLANG_ALLOW_OVERWRITE_LONGER_CONTEXT_LEN=1`），nonthink，13 任务 × 50 例，64K 并发 8 / 128K 并发 4，turboeval（同 §2/§3/§4.2.0 harness）。**YaRN 是独立 protocol 块，数字不与 8K/32K（原生 rope）表混排**；D−E/E−A 臂间差值在块内严格可比。均值为 12 任务（去 vt）。
